...
TreeNode* createBSTnode(TreeNode*root,int data){
    ...
    if(data>root->data)root->right=createBSTnode(root->right,data);
    if(data<root->data)root->left=createBSTnode(root->left,data);
    return root;
}
...
void levelOrderTraverselWithMarker(TreeNode*root){ //this is with marker that each level element in each line
